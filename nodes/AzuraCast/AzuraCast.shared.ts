...
function createProperties(
	...
	return [
		...
		},
		{
			...
			default: '{}',
			description:
				'Provide values for path placeholders, for example {"station_id":"demo","queue_item_id":1}',
		},
		{
...
function getDomainOperations(tag: string) {
	...
	const operationOptions = operations.map((operation) => ({
		name: `${operation.id} | ${operation.method} ${operation.path}`,
		value: operation.id,
		description: operation.summary || operation.description || operation.tag,
	}));
...
export function createDomainOperationConfig(tag: string) {
	...
	return {
		...
		properties: createProperties(operations.operationOptions, operations.defaultOperationId),
	};
}
